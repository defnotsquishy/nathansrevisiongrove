...
await build({configFile:false,logLevel:'error',build:{ssr:resolve('tests/entry.ts'),outDir:resolve('work/tests'),emptyOutDir:true}});
...
const done=m.finishSession(state,plan.sessions[0],3,'Remember both terms.',25,today);
assert.equal(done.logs.length,1);
assert.equal(m.finishSession(done,plan.sessions[0],3,'Duplicate',25,today).logs.length,1);
const file=new File([JSON.stringify({format:'revision-grove',version:1,state:done})],'backup.json',{type:'application/json'});
...
await assert.rejects(()=>m.readBackup(new File(['not json'],'invalid.json')));
...
assert.throws(()=>m.writeDevicePlan(state,m.readDevicePlan().revision),/could not save/);
const titles=new Set();
for(const page of Object.values(m.pageInfo)){
  const html=await readFile(resolve('dist-pages',page.path,'index.html'),'utf8');
  ...
  for(const resource of html.matchAll(/(?:src|href)="(\/nathansrevisiongrove\/[^"#?]*)"/g)){
    let file=resource[1].replace('/nathansrevisiongrove/','');if(!file||file.endsWith('/'))file+='index.html';
    assert.ok((await stat(resolve('dist-pages',file))).isFile(),file);
  }
}
for(const name of ['404.html','robots.txt','sitemap.xml','favicon.svg','.nojekyll'])assert.ok((await stat(resolve('dist-pages',name))).isFile());
for(const name of await readdir('dist-pages/assets'))if(name.endsWith('.js'))assert.ok((await stat(resolve('dist-pages/assets',name))).size<500000,`Bundle too large: ${name}`);
console.log('Passed: timetable scheduling, duplicate completion, device persistence, conflicts, backup validation/recovery, storage failures, seven static pages, metadata, links and bundle size limits.');
